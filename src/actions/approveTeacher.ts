...
export default () => {
    return async (ctx: Context) => {
        ...
        try {
            await api.teachers.update(id, { state: 'approved' });

            await ctx.telegram.editMessageText(
                message.chat.id,
                message.message_id,
                inline_message_id,
                `<b>Додавання викладача ${id} схвалено.</b>\n\n` +
                ...
                `<b>Коли:</b> ${new Date().toISOString()}`,
                {
                    ...
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: 'Скасувати та видалити', callback_data: `deny_teacher:${id}` }]
                        ],
                    },
                }
            );
            ...
            if (axiosError.isAxiosError) {
                if (axiosError.response?.status === 404) {
                    await ctx.telegram.editMessageText(
                        ...
                        message.message_id,
                        inline_message_id,
                        `<b>Викладача ${id} вже було видалено.</b>`,
                        {
                            parse_mode: 'HTML',
                        }
                    );
