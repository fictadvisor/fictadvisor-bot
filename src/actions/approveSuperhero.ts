...
export default () => {
    return async (ctx: Context) => {
        ...
        try {
            await api.superheroes.update(id, { state: 'approved' });

            await ctx.telegram.editMessageText(
                message.chat.id, 
                message.message_id,
                inline_message_id,
                `<b>Заявка на супергероя ${id} схвалена.</b>\n\n` +
                ...
                `<b>Коли:</b> ${new Date().toISOString()}`,
                {
                    ...
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: 'Скасувати та видалити', callback_data: `deny_superhero:${id}` }]
                        ],
                    },
                }
            );
