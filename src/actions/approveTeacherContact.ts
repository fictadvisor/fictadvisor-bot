...
export default () => {
    return async (ctx: Context) => {
        ...
        try {
            await api.contacts.update(id, { state: 'approved' });

            await ctx.telegram.editMessageText(
                message.chat.id,
                message.message_id,
                inline_message_id,
                `<b>Додавання контакту ${id} схвалено.</b>\n\n` +
                ...
                `<b>Коли:</b> ${new Date().toISOString()}`,
                {
                    ...
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: 'Скасувати та видалити', callback_data: `deny_contact:${id}` }]
                        ],
                    },
                }
            );
            ...
            if (axiosError.isAxiosError) {
                if (axiosError.response?.status === 404) {
                    await ctx.telegram.editMessageText(
                        ...
                        message.message_id,
                        inline_message_id,
                        `<b>Контакт ${id} вже було видалено.</b>`,
                        {
                            parse_mode: 'HTML',
                        }
                    );
